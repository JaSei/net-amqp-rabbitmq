...
our $VERSION = '0.02';
################################################################################
# based on https://github.com/emarcotte/net-amqp-RabbitMQ
#
# 0.02	similar API and return value as Net::RabbitMQ
################################################################################
use strict;
use warnings;
...
sub connect {
	...
	try {
		...
		if( $$options{timeout} ) {
			Time::HiRes::alarm( $$options{timeout} );
		}

		$self->_set_handle(
			IO::Socket::INET->new(
				PeerAddr => $hostname,
				PeerPort => $$options{port} || 5672,
				Proto => 'tcp',
	...
	my $password = $$options{password} || 'guest';
	my $username = $$options{username} || 'guest';
	my $virtualhost = $$options{virtual_host} || '/';
	my $heartbeat = $$options{heartbeat} || 0;
...
#
#In scalar context, this method returns the queuename delcared (important for retrieving the autogenerated queuename in the event that one was requested).
#
	...
	return $self->rpc_request(
		...
		output => [
			 Net::AMQP::Protocol::Queue::Declare->new(
				...
				arguments => {
					$self->_default( 'x-expires', $$arguments{ expires } ),
					$self->_default( 'x-message-ttl', $$arguments{ message_ttl } ),
				},
			),
		],
